...
const ReceiptForm = ({ onBack }) => {
  ...
  const [receiptModes] = useState(['Cash', 'Bank', 'Cheque', 'Draft']);
  ...
  const loadNextVoucherNumber = async () => {
    try {
      ...
      setFormData(prev => ({ ...prev, voucher_no: 'REC000001/2025-26' }));
    } catch (err) {
  ...
  async function handleSave() {
    try {
      ...
      // This will be implemented when API is integrated
      console.log('Saving receipt entry:', formData);
      ...
      setError(err.message || 'Failed to save receipt entry');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="accounting-form-container">
      ...
      <div className="form-header">
        ...
        <div className="ascii-line">│ Receipt No : {formData.voucher_no.padEnd(11)} Date : {formData.entry_date} Last Entry : {new Date().toLocaleDateString().padEnd(8)}│</div>
        <div className="ascii-border">├──────────────────────────────────────────────────────────────┤</div>
      </div>
      ...
      >
        ...
        <div className="entry-fields">
          ...
          <div className="field-row">
            ...
            <select
              name="receipt_mode"
              value={formData.receipt_mode}
              onChange={(e) => handleInputChange('receipt_mode', e.target.value)}
              className="form-input"
            >
