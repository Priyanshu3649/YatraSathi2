// Contra Form Component - Cash to Bank / Bank to Cash transfers
...
const ContraForm = ({ onBack }) => {
  ...
  const [ledgerList, setLedgerList] = useState(['Cash', 'Bank']);
  ...
  const loadNextVoucherNumber = async () => {
    try {
      ...
      setFormData(prev => ({ ...prev, voucher_no: 'CON000001/2025-26' }));
    } catch (err) {
  ...
  async function handleSave() {
    try {
      ...
      // Validation
      if (!formData.ledger_from || !formData.ledger_to) {
        setError('Both From and To ledgers are required');
        return;
      }
      ...
      // This will be implemented when API is integrated
      console.log('Saving contra entry:', formData);
  ...
  return (
    <div className="accounting-form-container">
      ...
      <div className="form-header">
        ...
        <div className="ascii-line">│ Contra No : {formData.voucher_no.padEnd(12)} Date : {formData.entry_date} Last Entry : {new Date().toLocaleDateString().padEnd(8)}│</div>
        <div className="ascii-border">├──────────────────────────────────────────────────────────────┤</div>
      </div>
      ...
      >
        ...
        <div className="entry-fields">
          <div className="field-row">
            ...
            <select
              name="ledger_from"
              value={formData.ledger_from}
              onChange={(e) => handleInputChange('ledger_from', e.target.value)}
          ...
          <div className="field-row">
            ...
            <select
              name="ledger_to"
              value={formData.ledger_to}
              onChange={(e) => handleInputChange('ledger_to', e.target.value)}
              className="form-input"
            >
      ...
      {(saveConfirmationOpen || globalSaveOpen) && (
        <SaveConfirmationModal
          ...
          onCancel={() => {}}
          message="Save this contra entry?"
        />
      )}
    </div>
  );
